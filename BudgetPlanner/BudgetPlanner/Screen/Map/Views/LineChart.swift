...
open class LineChart: UIView {
    ...
    public struct Coordinate {
        ...
        var linear: LinearScale!
        var scale: ((CGFloat) -> CGFloat)!
        var invert: ((CGFloat) -> CGFloat)!
        var ticks: (CGFloat, CGFloat, CGFloat)!
    }
    ...
    open var lineChartWidth: CGFloat = 2
    open var widthPerItemX: CGFloat = 60
    
    open var x: Coordinate = Coordinate()
    open var y: Coordinate = Coordinate()
    open var viewYAxis: UIView = UIView()
    ...
    fileprivate var dotsDataStore: [[DotView]] = []
    fileprivate var lineLayerStore: [CAShapeLayer] = []
    fileprivate var lineBezierPathStore: [UIBezierPath] = []
...
//        self.drawingWidth = self.bounds.width - (2 * x.axis.inset)
        self.drawingWidth = CGFloat(self.x.labels.values.count) * widthPerItemX - (2 * x.axis.inset)
...
//        handleTouchEvents(touches as NSSet?, event: event!)
        ...
        if lineBezierPathStore.count == 0 {
            return
        }
        for n in 0...lineBezierPathStore.count - 1 {
            let lineBezierPath = lineBezierPathStore[n]
            let dashedCGPath = lineBezierPath.cgPath.copy(strokingWithWidth: 10, lineCap: .butt, lineJoin: .miter, miterLimit: 0)
            if dashedCGPath.contains(point) {
                print ("We tapped plan ", n)
                delegate?.didSelectLine(n)
                return
            }
        }
    }
     ...
     */
    fileprivate func highlightDataPoints(_ point: DotView) {
        // make all dots old color
        for (lineIndex, dotsData) in dotsDataStore.enumerated() {
            for dot in dotsData {
                dot.backgroundColor = colors[lineIndex]
            }
        }
        
        guard let index = point.index else { return }
        guard let lineIndex = point.lineIndex else { return }
        let dotsData = dotsDataStore[lineIndex]
        ...
        dot.backgroundColor = Helpers.lightenUIColor(colors[lineIndex])
    }
    ...
    fileprivate func drawDataDots(_ lineIndex: Int) {
        ...
        for index in 0..<data.count {
            let xValue = self.x.scale(CGFloat(index)) + x.axis.inset - dots.radius/2
            let yValue = self.bounds.height - self.y.scale(data[index]) - y.axis.inset - dots.radius/2
            
            // draw custom layer with another layer in the center
            let button = DotView(frame: CGRect(x: xValue, y: yValue, width: dots.radius, height: dots.radius))
            button.index = index
            button.lineIndex = lineIndex
            button.addTarget(self, action: #selector(dotClicked), for: .touchUpInside)
            button.backgroundColor = colors[lineIndex]
            button.layer.cornerRadius = dots.radius / 2
            self.addSubview(button)
            dotViews.append(button)
        }
        dotsDataStore.append(dotViews)
    }
    
    @objc func dotClicked(_ sender: DotView) {
        print("click dot ", sender.index! , "of plan ", sender.lineIndex!)
        highlightDataPoints(sender)
        delegate?.didSelectDataPoint(sender.index!, lineIndex: sender.lineIndex!)
    }
...
//        let width = self.bounds.width
        let width = CGFloat(self.x.labels.values.count) * widthPerItemX
...
//        path.addLine(to: CGPoint(x: x.axis.inset, y: y.axis.inset))
        ...
        layer.strokeColor = x.axis.color.cgColor
        layer.fillColor = nil
        layer.lineWidth = 1
...
//        path.stroke()
    ...
    fileprivate func drawLine(_ lineIndex: Int) {
        ...
        layer.fillColor = nil
        layer.lineWidth = lineChartWidth
        self.layer.addSublayer(layer)
        ...
        lineBezierPathStore.append(path)
    ...
    fileprivate func drawXGrid() {
        self.layer.sublayers?.forEach { if $0.name == "X" {$0.removeFromSuperlayer()} }
        let path = UIBezierPath()
        var x1: CGFloat
        let y1: CGFloat = self.bounds.height - y.axis.inset
        let y2: CGFloat = y.axis.inset
        let (start, stop, step) = (CGFloat(0), CGFloat(x.labels.values.count - 1), CGFloat(1))
...
//        path.stroke()
    ...
    fileprivate func drawYGrid() {
        self.layer.sublayers?.forEach { if $0.name == "Y" {$0.removeFromSuperlayer()} }
        let path = UIBezierPath()
        let x1: CGFloat = x.axis.inset
//        let x2: CGFloat = self.bounds.width - x.axis.inset
        let x2: CGFloat = CGFloat(self.x.labels.values.count) * widthPerItemX - x.axis.inset
        var y1: CGFloat
...
//            self.addSubview(label)
//        }
//        // add unit title y label ($)
//        let label = UILabel(frame: CGRect(x: 0, y: 0, width: y.axis.inset + 3, height: y.axis.inset))
//        label.font = UIFont.preferredFont(forTextStyle: UIFont.TextStyle.caption2)
...
//        self.addSubview(label)
        ...
        let (start, stop, step) = self.y.ticks
        for i in stride(from: start, through: stop, by: step) {
            yValue = self.viewYAxis.bounds.height - self.y.scale(i) - (y.axis.inset * 1.5)
            let label = UILabel(frame: CGRect(x: 0, y: yValue, width: 45, height: y.axis.inset))
            label.font = UIFont.preferredFont(forTextStyle: UIFont.TextStyle.caption2)
            label.textAlignment = .center
            label.text = String(Int(round(i))) + " $"
            listLabel.append(label)
        }
        delegate?.drawYLabels(listLabel)
    }
    ...
    open func clear() {
        // clear data
        dataStore.removeAll()
        lineBezierPathStore.removeAll()
        self.setNeedsDisplay()
    }
}

class DotView: UIButton {
    var index: Int?
    var lineIndex: Int?
